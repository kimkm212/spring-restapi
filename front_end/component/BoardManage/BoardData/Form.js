...
class Form extends Component {
  ...
  _saveAction() {
    axios.post(`http://localhost:8080/api/boardData/${this.state.boardId}`,
        this.state
    ).then((res) => {
  ...
  _updateAction() {
    axios.put(`http://localhost:8080/api/boardData/${this.state.boardId}`,
        this.state
    ).then((res) => {
  ...
  render() {
    return (
        <div className="ui form">
          ...
          <button className="ui button" type="button" onClick={this._action.bind(this)}>등록</button>
